...
表示は何行でも増やせて、各行の左側と右側をそれぞれテンプレートとして書きます。各行には折り返しの行数も選べて、左側だけがその行数まで折り返します。
...
通知の title と subtitle にもテンプレートを 1 つずつ書けます。body はテンプレートのリストで、空になった行は出ません。`{agent_icon}` は何も出ません。
